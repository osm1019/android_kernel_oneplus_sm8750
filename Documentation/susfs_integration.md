...
`v2.5.0`.  The susfs kernel patch set is selected for Linux `6.6`.
...
| Option | Description |
| --- | --- |
| `--kernel-repo DIR` | Override the kernel root (defaults to the current repo). |
...
1. Ensures the kernel root (this repository) contains `common/` as required by
   the KernelSU build guide.
...
4. Copies the following payload into your tree:
   ...
   * `kernel_patches/50_add_susfs_in_kernel-<kernel_version>.patch` →
     `common/50_add_susfs_in_kernel.patch`.
   * `kernel_patches/fs/*` → `common/fs/`.
   * `kernel_patches/include/linux/*` → `common/include/linux/`.
5. Applies the KernelSU susfs enablement patch (`patch -p1` inside `KernelSU/`).
6. Applies the kernel susfs integration patch (`patch -p1` inside `common/`).

Once the script finishes, proceed with the standard KernelSU build flow under
`common/` (or Bazel if you already have a workflow established).  The cloned
